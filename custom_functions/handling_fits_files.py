...
def open_ifu_fits(filename, instrument='muse', data_type='cube', quiet_val=False):
	...
	if ('muse' in instrument):
		print_cust('Loading MUSE-IFU data...', quiet_val=quiet_val)
		...
		print_cust('MUSE-IFU data loaded...', quiet_val=quiet_val)
		...
		if ('spectra' in data_type):
			print_cust('Loading SDSS spectra...', quiet_val=quiet_val)
			with fits.open(str(filename)) as hdul:
			...
			print_cust('SDSS spectra loaded...', quiet_val=quiet_val)
		elif ('image' in data_type):
			print_cust('Loading SDSS image...', quiet_val=quiet_val)
			with fits.open(str(filename)) as hdul:
				header_original = hdul[0].header
				header_original_err = hdul[1].header
...
def open_ifu_fits_custom_file(filename, instrument='muse', quiet_val=False):
	instrument=instrument.lower()
	if ('muse' in instrument):
		print_cust('Custom loading IFU data...', quiet_val=quiet_val)
		assert os.path.exists(filename), f"File: {filename} not found..."
		...
		#sky_original_file, header_original_sky = astropygetdata(filename, 6, header=True)
		print_cust('Custom IFU data loaded...', quiet_val=quiet_val)
	...
	if ('muse' in instrument):
		if (os.path.exists(file_name_new)):
			...
			if ('y' in prompt_1.lower()):
				print_cust(f"deleting...{file_name_new}", quiet_val=quiet_val)
				string_for_deleting = str('rm ') + file_name_new
				print_cust(string_for_deleting, quiet_val=quiet_val)
				...
				print_cust('Overwrite skipped. Returning saved file information', quiet_val=quiet_val)
		else:
			with fits.open(filename_old) as hdu_list:
				hdu_list.writeto(file_name_new)  # to write all HDUs, including the updated one, to a new file
		...
		print_cust(f"Please define opening method for the instrument: {instrument} first", quiet_val=quiet_val)
########################################HANDLING_IFU_FITS########################################
...
def modify_ifu_fits(filename_old, file_name_new, wave_new, data_new, **kwargs):
	...
	crval_cust, naxis_cust, cdn_n, cr_pix = retrieve_wave_header_info(wave_new, wave_type='log')
	if ('muse' in instrument):
		if (os.path.exists(file_name_new)):
			prompt_1=input(f"File {file_name_new} already exists. Overwrite?(y/n) : ")
			if ('y' in prompt_1.lower()):
				...
				print_cust('-Saving data and err', quiet_val=quiet_val)
				...
				print_cust('-Saved data and err', quiet_val=quiet_val)
				print_cust('-Saving continuum', quiet_val=quiet_val)
				...
				print_cust('-Saved continuum', quiet_val=quiet_val)
				print_cust('-Saving FWHM', quiet_val=quiet_val)
				...
				print_cust('-Saved FWHM', quiet_val=quiet_val)
				print_cust('-Saving Velocity Scale', quiet_val=quiet_val)
				...
				print_cust('-Saved Velocity Scale', quiet_val=quiet_val)
				print_cust('-Saving Sky Spectra', quiet_val=quiet_val)
				...
				print_cust('-Saved Sky Spectra', quiet_val=quiet_val)
				print_cust(f"New file: {file_name_new} saved", quiet_val=quiet_val)
			else:
				print_cust('Overwrite skipped. Returning saved file information', quiet_val=quiet_val)
		else:
			header_new, header_new_err, data_new_file, err_new_file = copy_ifu_fits(filename_old, file_name_new, instrument=instrument)
...
def obtain_physical_axis(header_original, **kwargs):
	...
	if (('sdss' in instrument) and ('image' in data_type)):
		print_cust(r'Assuming SDSS-IMAGE coordinate system(RA(deg), Dec(deg))', quiet_val=quiet_val)
	...
	if (('sdss' in instrument) and ('spectra' in data_type)):
		print_cust(r'Assuming SDSS-spectral coordinate system(Wavelength(Ang))', quiet_val=quiet_val)
	...
	if (('MUSE' in instrument) and ('cube' in data_type)):
		print_cust(r'Assuming VLT-MUSE coordinate system(RA(deg), Dec(deg), Wavelength(Ang))', quiet_val=quiet_val)
		if (obtain_type=='wcs'):
			print_cust('Retrieving physical axis from astropy.wcs.WCS', quiet_val=quiet_val)
			...
			print_cust('Retrieving physical axis from manual code by Axe(05/10/2022)', quiet_val=quiet_val)
			...
			print_cust('Type for obtaining physical axes unidentified. Quitting...', quiet_val=quiet_val)
			quit()
########################################OBTAIN_PHYSICAL_AXIS_FROM_HEADER########################################
