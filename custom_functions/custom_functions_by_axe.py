...
basic_functions.quite_val = quiet_val

from plotting_functions import *
import plotting_functions
plotting_functions.quite_val = quiet_val

from get_data_from_file import *
import get_data_from_file
get_data_from_file.quite_val = quiet_val

from handling_fits_files import *
import handling_fits_files
handling_fits_files.quite_val = quiet_val

from spatial_binning_functions import *
import spatial_binning_functions
spatial_binning_functions.quite_val = quiet_val

from emission_fit_function import *
import emission_fit_function
emission_fit_function.quite_val = quiet_val

from extinction_fitting_functions import *
import extinction_fitting_functions
extinction_fitting_functions.quite_val = quiet_val

from ppxf_fitting_functions import *
import ppxf_fitting_functions
ppxf_fitting_functions.quite_val = quiet_val
...
def stage_one_analysis(par_dict, new_filename, quiet_val=False):
	...
	print_cust('Performing STAGE-I analysis...', quiet_val=quiet_val)
	#LOAD ORIGINAL MUSE CUBE
	print_cust('Loading Original Data...', quiet_val=quiet_val)
	header_original, header_original_err, data_original_file, err_original_file = open_ifu_fits(str(par_dict['muse_data_filename']), str(par_dict['observed_instrument']))
	print_cust('Original Data Loaded...', quiet_val=quiet_val)
	#OBTAINING PHYSICAL AXES
	print_cust('Obtaining Physical Axes...', quiet_val=quiet_val)
	ra_array, dec_array, wave_array = obtain_physical_axis(header_original)
	print_cust('Physical Axes Retrieved...', quiet_val=quiet_val)
	#CLEANING DATA, REMOVING NANs, EMPTY ELEMENTS, etc.
	print_cust('Cleaning Data...', quiet_val=quiet_val)
	data_original_cleaned_file = clean_data(data_original_file)
	err_original_cleaned_file = clean_data(err_original_file, type_of_data='err')
	print_cust('Data Cleaned...', quiet_val=quiet_val)
	#Flattening data for convinience
	print_cust('Flattening Original Data...', quiet_val=quiet_val)
	...
	print_cust('Original Data Flattened...', quiet_val=quiet_val)
	#Rebinning to logscale, spectral smoothing, sky and continuum estimation
	print_cust('Rebinning to logscale, spectral smoothing, sky and continuum estimation...', quiet_val=quiet_val)
	wave_rebinned_central, data_rebinned_central, data_err_rebinned_central, fwhm_gal_central, velscale_central = refine_obs_data_using_scipy(wave_array, data_flattened[:, int(data_flattened.shape[1]/2)], data_err_flattened[:, int(data_flattened.shape[1]/2)], spectral_smoothing=int(par_dict['spectral_smoothing']))
	cont_init_central = get_initial_continuum_rev_2(wave_rebinned_central, data_rebinned_central)
	wave_sky_full_rebinned, flux_sky_full_rebinned, flux_err_sky_full_rebinned = check_for_sky(par_dict, wave_rebinned_central)
	...
	#for i in range(0, data_flattened.shape[1], 100):
		bar.next()
		data_rebinned[:, i], data_err_rebinned[:,i], fwhm_gal_datacube[:, i], velscale_datacube[i], continuum_datacube[:, i] = combined_function_rebinning_continuum(wave_array, data_flattened[:, i], data_err_flattened[:, i], redshift_val=float(par_dict['redshift_val']), quiet_val=quiet_val)
	print_cust('Rebinning to logscale, spectral smoothing, sky and continuum estimation is now complete...', quiet_val=quiet_val)
	#Recovering new data back to the 3D shape
	print_cust('Recovering new data back to the 3D shape...', quiet_val=quiet_val)
	...
	print_cust('3D shape for new data recovered...', quiet_val=quiet_val)
	#Saving revised data
	print_cust('Saving revised data...', quiet_val=quiet_val)
	modify_ifu_fits(str(par_dict['muse_data_filename']), new_filename, wave_rebinned_central, data_rebinned_reshaped, data_err_cube=data_err_rebinned_reshaped, data_cont_cube=continuum_datacube_reshaped, data_fwhm_cube=fwhm_gal_datacube_reshaped, data_velscale_image=velscale_datacube_reshaped, sky_array=flux_sky_full_rebinned)
	print_cust('Revised data saved...', quiet_val=quiet_val)
	print_cust('STAGE-I analysis complete...', quiet_val=quiet_val)
	print_cust(f"---STAGE-I took {float(time.time() - start_time1)} seconds ---", quiet_val=quiet_val)
...
def stage_two_analysis(par_dict, new_filename, file_name_eq_width, quiet_val=False):
	...
	print_cust('Performing STAGE-II analysis...', quiet_val=quiet_val)
	#To obtain information from revised data
	print_cust('Loading information from revised data...', quiet_val=quiet_val)
	...
	print_cust('information from revised data loaded...', quiet_val=quiet_val)
	#print_cust(f'{wave_rev.min()}, {wave_rev.max()}', quiet_val=quiet_val)
	print_cust('Making EW Maps...', quiet_val=quiet_val)
	print_cust('Flattening and Normalising Data...', quiet_val=quiet_val)
	...
	print_cust('Data Flattened and Normalised...', quiet_val=quiet_val)
	...
	print_cust(f"Estimating EW for: {lick_index_species_rev}", quiet_val=quiet_val)
	...
	#for i in range(0, data_flattened_normalised.shape[1], 100):
		...
		for j in range(len(mean_wave_list_rev)):
			ew_map_array[j, i, 0], ew_map_array[j, i, 1] = get_equivalendth_width_rev(wave_rest, data_flattened_normalised[:, i], data_err_flattened_normalised[:, i], center=float(mean_wave_list_rev[j]), redshift=float(par_dict['redshift_val']))
	...
	header_ew, header_ew_err, data_ew_file, err_ew_file = copy_ifu_fits(str(par_dict['muse_data_filename']), file_name_eq_width)
	...
	print_cust('EW Maps generated...', quiet_val=quiet_val)
	print_cust('STAGE-II analysis complete...', quiet_val=quiet_val)
	print_cust(f"---STAGE-II took {float(time.time() - start_time2)} seconds ---", quiet_val=quiet_val)
...
def stage_three_analysis(file_name_rev_linear, dir_name_4, par_dict, display_binning=False, quiet_val=False, region_of_binning_interest=[0, 0, -1, -1], region_of_binning_interest_type='logical'):
	...
	if 'voronoi' in str(par_dict['binning_type'].lower()):
		...
		file_name_rev_binned = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_binned_') + str(par_dict['binning_type']) + str('_snr_type_') + str(par_dict['voronoi_snr_type']) + str('_quant_') + str(par_dict['binning_quant']) + str('.dat')
		voronoi_binning_func(file_name_rev_linear, file_name_rev_binned, int(par_dict['binning_quant']), str(par_dict['voronoi_snr_type']), quiet_val=False, region_of_binning_interest=region_of_binning_interest, region_of_binning_interest_type=region_of_binning_interest_type)
		...
		file_name_rev_binned = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_binned_') + str(par_dict['binning_type']) + str('_ellipse_type_') + str(par_dict['ellipse_angle_for_advanced_binning']) + str('_quant_') + str(par_dict['binning_quant']) + str('.dat')
		advanced_binning_func(file_name_rev_linear, file_name_rev_binned, par_dict, region_of_binning_interest=region_of_binning_interest, region_of_binning_interest_type=region_of_binning_interest_type)
	elif 'square' in str(par_dict['binning_type'].lower()):
		assert ('binning_quant' in par_dict), f"Binning quant required..."
		file_name_rev_binned = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_binned_') + str(par_dict['binning_type']) + str('_quant_') + str(par_dict['binning_quant']) + str('.dat')
		square_binning_func(file_name_rev_linear, file_name_rev_binned, par_dict, region_of_binning_interest=region_of_binning_interest, region_of_binning_interest_type=region_of_binning_interest_type)
	elif 'none' in str(par_dict['binning_type'].lower()):
		print_cust('Spatial Binning not required by user', quiet_val=quiet_val)
		file_name_rev_binned = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_no_spatial_binning.dat')
		no_spatial_binning_func(file_name_rev_linear, file_name_rev_binned, par_dict)
	else:
		print_cust('Spatial Binning not specified by user. Setting to no spatial binning.', quiet_val=quiet_val)
		file_name_rev_binned = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_no_spatial_binning.dat')
		no_spatial_binning_func(file_name_rev_linear, file_name_rev_binned, par_dict)
	if (display_binning):
		binning_result_display(file_name_rev_linear, file_name_rev_binned, par_dict)
	print_cust(f"---STAGE-III took {float(time.time() - start_time3)} seconds ---", quiet_val=quiet_val)
...
def stage_four_execution(par_dict, file_name_rev_linear, file_name_rev_binned, dir_name_4, wave_rev, halpha_eq_width_map, sky_rev_file, data_rev_file, err_rev_file, cont_rev_file, fwhm_rev_file, quiet_val=False):
	start_time4 = time.time()
	print_cust("Executing stage four...", quiet_val=quiet_val)
	if 'snr_map' in str(par_dict['execution_type'].lower()):
		print_cust('Getting SNR Map...', quiet_val=quiet_val)
		get_snr_map_revised(file_name_rev_linear, file_name_rev_binned, par_dict)
	if 'fit' in str(par_dict['execution_type'].lower()):
		print_cust('Fitting...', quiet_val=quiet_val)
		...
		if (os.path.exists(main_result_cube_filename)):
			...
			if ('y' in prompt_4.lower()):
				print_cust(f"deleting...{main_result_cube_filename}", quiet_val=quiet_val)
				string_for_deleting = str('rm ') + main_result_cube_filename
				print_cust(string_for_deleting)
				os.system(string_for_deleting)
				print_cust(f"deleting...{main_absorption_result_cube_filename}", quiet_val=quiet_val)
				string_for_deleting = str('rm ') + main_absorption_result_cube_filename
				print_cust(string_for_deleting)
				os.system(string_for_deleting)
				print_cust(f"deleting...{main_emission_result_cube_filename}", quiet_val=quiet_val)
			...
			fitting_regime(file_name_rev_binned, wave_rev, halpha_eq_width_map, sky_rev_file, data_rev_file, err_rev_file, cont_rev_file, fwhm_rev_file, main_result_cube, main_absorption_result_cube, main_emission_result_cube, main_result_cube_filename, main_absorption_result_cube_filename, main_emission_result_cube_filename, par_dict)
	print_cust("Stage four execution complete...", quiet_val=quiet_val)
	print_cust(f"---STAGE-IV took {float(time.time() - start_time4)} seconds ---", quiet_val=quiet_val)
	#return None
	#continue
########################################STAGE_FOUR_EXECUTION########################################
...
def fitting_regime(file_name_rev_binned, wave_rev, halpha_eq_width_map, sky_rev_file, data_rev_file, err_rev_file, cont_rev_file, fwhm_rev_file, main_result_cube, main_absorption_result_cube, main_emission_result_cube, main_result_cube_filename, main_absorption_result_cube_filename, main_emission_result_cube_filename, par_dict, quiet_val=False):
	print_cust("Starting fitting...", quiet_val=quiet_val)
	x_axis_binned_rev, y_axis_binned_rev, bin_num = np.loadtxt(file_name_rev_binned).T
	bin_num_unique = np.unique(bin_num)
	...
	halpha_eq_width_map[np.abs(halpha_eq_width_map)>100.]=np.nan
	...
	print_cust("Saving files...", quiet_val=quiet_val)
	np.save(main_result_cube_filename, main_result_cube)
	print_cust(f"File: {main_result_cube_filename} saved...", quiet_val=quiet_val)
	np.save(main_absorption_result_cube_filename, main_absorption_result_cube)
	print_cust(f"File: {main_absorption_result_cube_filename} saved...", quiet_val=quiet_val)
	np.save(main_emission_result_cube_filename, main_emission_result_cube)
	print_cust(f"File: {main_emission_result_cube_filename} saved...", quiet_val=quiet_val)
########################################FITTING_REGIME_FUNCTION########################################
...
def absorption_fit(wave_rev, flux, err, par_dict, **kwargs):
	...
	lam_gal_rev, fit_galaxy, fit_noise, fit_sol_pp, fit_miles, fit_component_rev, fit_gas_names, fit_normalising_factor = ppxf_function(file1, redshift=par_dict['redshift_val'], number_of_stellar_components=par_dict['ppxf_stars_comp'], number_of_gas_components=par_dict['ppxf_gas_comp'], sky_flux_init=flux_sky1, first_order_kinematic_limits=1000., tie_balmer=True, limit_doublets=True, quiet=quiet_val)
	bestfit_solution_array, stellar_spectrum_array, sol_pp_gas_flux_reshaped, sol_pp_gas_flux_err_reshaped, stellar_vel, stellar_vel_err, stellar_sigma, stellar_sigma_err, stellar_h3, stellar_h3_err, stellar_h4, stellar_h4_err, gas_balmer_vel, gas_balmer_vel_err, gas_balmer_sigma, gas_balmer_sigma_err, gas_others_vel, gas_others_vel_err, gas_others_sigma, gas_others_sigma_err, status_of_optimization, reddening_fit_e_b_minus_v, gas_reddening_fit_e_b_minus_v, goodpixels_idx, reduced_chi_sq, st_age_unique, st_mass_unique, st_lum_unique, weighted_logAge, weighted_metallicity, mlpop_sol, str_fit = get_detailed_ppxf_solution(fit_sol_pp, fit_miles, fit_component_rev, fit_gas_names, par_dict['ppxf_stars_comp'], par_dict['ppxf_gas_comp'], normalising_factor = fit_normalising_factor, st_age_unique_start=par_dict['st_age_list_start'], st_age_unique_end=par_dict['st_age_unique_end'], quiet=quiet_val)
...
def emission_fit(wave_rev, flux, err, par_dict, **kwargs):
	...
	if ('custom'in par_dict['emission_fit_type']):
		wave_fit, flux_fit, err_fit, continuum_fitted, res_fitted_full_fitted, masked_array, pfit_curvefit, perr_curvefit, line_flux, line_flux_err, red_chi_squared, amp_length = complex_fit_func(wave_rev, flux, err, par_dict, FWHM_gal = FWHM_gal1, flux_sky = flux_sky1, cont_init = init_cont1, quiet_val=quiet_val)
...
def get_output_filenames_and_arrays(dir_name_4, bin_num_unique, wave_rev, par_dict, quiet_val=True):
	...
	main_result_cube_filename = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_binned_') + str(par_dict['binning_type']) + str('_snr_type_') + str(par_dict['voronoi_snr_type']) + str('_quant_') + str(par_dict['binning_quant']) + str('_results_cube') + str('.npy')
	main_result_cube = np.zeros([len(bin_num_unique), len(wave_rev), 6])
	main_absorption_result_cube_filename = str(dir_name_4) + str('/') + str(par_dict['muse_data_filename']).split("/")[-1].replace('.fits', '') + str('_binned_') + str(par_dict['binning_type']) + str('_snr_type_') + str(par_dict['voronoi_snr_type']) + str('_quant_') + str(par_dict['binning_quant']) + str('_custom_absorption_results') + str('.npy')
	total_count_of_abs_pars = (par_dict['ppxf_stars_comp']*4) + 2 + (par_dict['ppxf_gas_comp']*2) + (len(par_dict['st_age_list_start'])*3) + 2
...
def stage_five_analysis(main_result_cube_filename, main_absorption_result_cube_filename, main_emission_result_cube_filename, par_dict, wave_rev, file_name_rev_binned, expanded_hdr_filename, expanded_filename, quiet_val=False):
	start_time7 = time.time()
	print_cust("Executing stage five...", quiet_val=quiet_val)
	if (os.path.exists(expanded_hdr_filename) or os.path.exists(expanded_filename)):
		...
		if ('y' in prompt_5.lower()):
			print_cust(f"deleting...{expanded_hdr_filename} and {expanded_filename}")
			string_for_deleting = str('rm ') + expanded_hdr_filename
			print_cust(string_for_deleting)
			os.system(string_for_deleting)
			string_for_deleting2 = str('rm ') + expanded_filename
			print_cust(string_for_deleting2)
			os.system(string_for_deleting2)
			expanded_hdr_filename1, expanded_filename1 = expand_results(main_result_cube_filename, main_absorption_result_cube_filename, main_emission_result_cube_filename, par_dict, wave_rev, file_name_rev_binned, expanded_hdr_filename, expanded_filename)
		else:
	...
	print_cust("Stage five execution complete...", quiet_val=quiet_val)
	print_cust(f"---STAGE-V took {float(time.time() - start_time7)} seconds ---", quiet_val=quiet_val)
	return(expanded_hdr_filename1, expanded_filename1)

def expand_results(main_result_cube_filename, main_absorption_result_cube_filename, main_emission_result_cube_filename, par_dict, wave_rev, file_name_rev_binned, expanded_hdr_filename, expanded_filename):
	...
	print_cust("Expansion successful")
	print_cust(f"Expanding data took {float(time.time() - start_time5)} seconds ---")
...
def get_expanded_data_information_emission(popt_fit, perr_fit, wave, par_dict, **kwargs):
	...
	amp_array, center_array, sigma_array, reddening_val_fit, coeff_fit = get_params(popt_fit, par_dict_rev['number_of_narrow_components'], par_dict_rev['number_of_wide_components'], par_dict_rev['center_list'], amp_length)
	...
	amp_err_array, center_err_array, sigma_err_array, reddening_err_val_fit, coeff_err_fit = get_params(perr_fit, par_dict_rev['number_of_narrow_components'], par_dict_rev['number_of_wide_components'], par_dict_rev['center_list'], amp_length)
	amp_err_array_rev = retrieve_all_amplitude_list_rev2(list(amp_err_array), par_dict_rev['number_of_narrow_components'], par_dict_rev['number_of_wide_components'], position_init_narrow_comp, position_init_wide_comp, position_final_narrow_comp, position_final_wide_comp, par_dict_rev['comments_on_tied'], par_dict_rev['comments_on_balmer'])
...
def get_expanded_header_information_emission(wave, par_dict, **kwargs):
	...
	popt_init, masked_array, amp_init_array, position_init_narrow_comp, position_init_wide_comp, position_final_narrow_comp, position_final_wide_comp, coeff_init_val, index_for_fixing_bounds_factor_init_narrow_comp, index_for_fixing_bounds_factor_init_wide_comp = get_opt_param_array_rev2(wave, flux, cont_init, par_dict_rev['number_of_narrow_components'], par_dict_rev['number_of_wide_components'], par_dict_rev['center_list'], par_dict_rev['redshift_val'], par_dict_rev['comments_on_balmer'], par_dict_rev['comments_on_tied'], par_dict_rev['factor_for_tied'], par_dict_rev['e_b_minus_v_init'], par_dict_rev['window_for_fit'], par_dict_rev['stopping_number_for_continuum'], par_dict_rev['center_init_array'], par_dict_rev['sigma_init_array'])
	amp_length = len(amp_init_array)
	amp_array, center_array, sigma_array, reddening_val_fit, coeff_fit = get_params(popt_init, par_dict_rev['number_of_narrow_components'], par_dict_rev['number_of_wide_components'], par_dict_rev['center_list'], len(amp_init_array))
	amp_array_rev = retrieve_all_amplitude_list_rev2(list(amp_array), par_dict_rev['number_of_narrow_components'], par_dict_rev['number_of_wide_components'], position_init_narrow_comp, position_init_wide_comp, position_final_narrow_comp, position_final_wide_comp, par_dict_rev['comments_on_tied'], par_dict_rev['comments_on_balmer'])
	...
	header_information_emission.extend([[r'$\sigma$']*len(sigma_array)])
	header_information_emission.extend(['E(B-V)(gas)'])
	header_information_emission = np.array(header_information_emission)
	header_information_err_emission = []
	header_information_err_emission.extend([['dAmp']*len(amp_array_rev)])
	header_information_err_emission.extend([['dV']*len(center_array)])
	header_information_err_emission.extend([[r'd$\sigma$']*len(sigma_array)])
	header_information_err_emission.extend(['dE(B-V)(gas)'])
	header_information_err_emission = np.array(header_information_err_emission)
	return (header_information_emission, header_information_err_emission, amp_length)
...
def get_expanded_information_abs(par_dict):
	...
	header_information_absorption.extend(['V', r'$\sigma$', 'h3', 'h4']*par_dict['ppxf_stars_comp'])
	header_information_absorption.extend(['E(B-V)', 'E(B-V)(gas)'])
	header_information_absorption.extend(['V(gas)', r'$\sigma$(gas)', 'V(other)', r'$\sigma$(other)']*par_dict['ppxf_gas_comp'])
	header_information_absorption.extend(par_dict['st_age_list_start'])
	header_information_absorption.extend(['mass']*len(par_dict['st_age_list_start']))
	header_information_absorption.extend(['lum']*len(par_dict['st_age_list_start']))
	header_information_absorption.extend(['StAge', 'StMetallicity'])
	header_information_absorption = np.array(header_information_absorption)

	header_information_err_absorption = []
	header_information_err_absorption.extend(['dV', r'$d\sigma$', 'dh3', 'dh4']*par_dict['ppxf_stars_comp'])
	header_information_err_absorption.extend(['SOP', r'$\chi^{2}$'])
	header_information_err_absorption.extend(['dV(gas)', r'd$\sigma$(gas)', 'dV(other)', r'd$\sigma$(other)']*par_dict['ppxf_gas_comp'])
	header_information_err_absorption.extend(par_dict['st_age_list_start'])
	header_information_err_absorption.extend(['mass']*len(par_dict['st_age_list_start']))
	header_information_err_absorption.extend(['lum']*len(par_dict['st_age_list_start']))
	header_information_err_absorption.extend(['ML_sol', 'NA'])
	header_information_err_absorption = np.array(header_information_err_absorption)
	return (header_information_absorption, header_information_err_absorption)
