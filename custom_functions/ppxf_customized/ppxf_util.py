...
def emission_lines_rev_sulphur_II(logLam_temp, lamRange_gal, FWHM_gal, pixel=True,
                   tie_balmer=False, limit_doublets=False, vacuum=False, quiet_val=True):
    ...
    if not quiet_val: print('Emission lines included in gas templates:')
    if not quiet_val: print(line_names)
    return emission_lines, line_names, line_wave

def emission_lines_rev3(logLam_temp, lamRange_gal, FWHM_gal, pixel=True,
                   tie_balmer=False, limit_doublets=False, vacuum=False, quiet_val=True):
    if tie_balmer:
        ...
        line_wave = wave[-2] if np.any(w) else np.mean(wave)
    else:
...
def emission_lines_rev2(logLam_temp, lamRange_gal, FWHM_gal, pixel=True,
                   tie_balmer=False, limit_doublets=False, vacuum=False, quiet_val=True):
    if tie_balmer:
    ...
    #'''
    #Extra species added by Axe, date:08032022, comment out or change accordingly
    '''
    #       -----HeII4685-----
    wave = [4685.710]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    doublet = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, doublet])
    line_names = np.append(line_names, 'HeII4685')  # single template for this doublet
    line_wave = np.append(line_wave, wave)

    #       -----[Fe VII]-----
    wave = [5158.890, 5720.700, 6087.000]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    names = ['[FeVII]5158', '[FeVII]5720', '[FeVII]6087']
    gauss = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, gauss])
    line_names = np.append(line_names, names)
    line_wave = np.append(line_wave, wave)
    ...
    #       -----[N I]-----
    wave = [5200.257]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    doublet = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, doublet])
    line_names = np.append(line_names, '[NI]5200')  # single template for this doublet
    line_wave = np.append(line_wave, wave)


    #       -----O I-----
    wave = [7002.230]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    doublet = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, doublet])
    line_names = np.append(line_names, 'OI7002')  # single template for this doublet
    line_wave = np.append(line_wave, wave)

    #       -----[Ar V]-----
    wave = [7005.870]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    doublet = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, doublet])
    line_names = np.append(line_names, '[ArV]7005')  # single template for this doublet
    line_wave = np.append(line_wave, wave)
    '''

    #       -----[N II]-----
    wave = [5754.590]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    doublet = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, doublet])
    line_names = np.append(line_names, '[NII]5754')  # single template for this doublet
    line_wave = np.append(line_wave, wave)

    '''
    #       -----He I-----
    wave = [5875.624]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    doublet = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, doublet])
    line_names = np.append(line_names, 'HeI5875')  # single template for this doublet
    line_wave = np.append(line_wave, wave)


    #       -----[Ar IV]-----
    wave = [4711.260, 4740.120]    # air wavelengths
    if vacuum:
        wave = air_to_vac(wave)
    names = ['[ArIV]4711', '[ArIV]4740']
    gauss = gaussian(logLam_temp, wave, FWHM_gal, pixel)
    emission_lines = np.column_stack([emission_lines, gauss])
    line_names = np.append(line_names, names)
    line_wave = np.append(line_wave, wave)
...
def emission_lines_rev(logLam_temp, lamRange_gal, FWHM_gal, pixel=True,
...
def emission_lines(logLam_temp, lamRange_gal, FWHM_gal, pixel=True,
                   tie_balmer=False, limit_doublets=False, vacuum=False, quiet=False):
    """
    Generates an array of Gaussian emission lines to be used as gas templates in PPXF.
