...
#   V1.0.3: Removed dependency on cap_readcol. MC, Oxford, 10 May 2018
    ...
    def mass_to_light(self, weights, band="r", quiet=False):
        ...
        mlpop = np.nansum(weights*mass_no_gas_grid)/np.nansum(weights*lum_grid)
...
##############################################################################
    ...
    def mean_age_metal(self, weights, quiet=False):
        ...
        mean_log_age = np.nansum(weights*log_age_grid)/np.nansum(weights)
        mean_metal = np.nansum(weights*metal_grid)/np.nansum(weights)
