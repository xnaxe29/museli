...
def no_spatial_binning_func_main(input_filename, output_filename, physical_axes=False, quiet_val=False):
	bf.print_cust(f"Loading linear data file: {input_filename} - ", quiet_val=quiet_val)
	...
	for i in range(0, len(x_lin), 1):
		...
		count+=1
	bf.print_cust(f"Number of bins created: {count}", quiet_val=quiet_val)
	...
	bf.print_cust(f"Binning Saved in file: {output_filename}", quiet_val=quiet_val)
def no_spatial_binning_func(input_filename, output_filename, par_dict, physical_axes_val=False, quiet_val=False):
	if (os.path.isfile(output_filename)):
		...
		if (save_prompt6=='y'):
			bf.print_cust('Setting linear file as binned file', quiet_val=quiet_val)
			no_spatial_binning_func_main(input_filename, output_filename, physical_axes=physical_axes_val)
			bf.print_cust('Saved...', quiet_val=quiet_val)
		else:
			bf.print_cust('Exiting binning process....', quiet_val=quiet_val)
	else:
		bf.print_cust('Setting linear file as binned file', quiet_val=quiet_val)
		no_spatial_binning_func_main(input_filename, output_filename, physical_axes=physical_axes_val)
		bf.print_cust('Saved...', quiet_val=quiet_val)
...
def advanced_binning_func_basic(input_filename, output_filename, ellipse_angle, bin_chopping_angle, binning_radius_list, center_coordinates, physical_axes=False, quiet_val=False, region_of_binning_interest=[0, 0, -1, -1], region_of_binning_interest_type='logical'):
	...
	if (bin_chopping_angle!=0 and binning_radius_list):
		...
		bf.print_cust(f"Warning! No specific bin chipping angle or radius list for ellipse defined.", quiet_val=quiet_val)
...
def voronoi_binning_func(input_filename, output_filename, targetSN, snr_type, plot_val=False, quiet_val=True, physical_axes=False, region_of_binning_interest=[0, 0, -1, -1], region_of_binning_interest_type='logical'):
	...
	bf.print_cust(f"Target SNR: {targetSN}", quiet_val=quiet_val)
	...
	if ('ew' in snr_type.lower()):
		...
		noise = noise1
	snr = signal/noise
	bf.print_cust(f"SNR/EW Limits: {np.min(snr)} to {np.max(snr)}", quiet_val=quiet_val)
	if (os.path.isfile(output_filename)):
		...
		if (save_prompt4=='y'):
			if (physical_axes):
				...
				binned_array1 = np.column_stack([x1[~mask], y1[~mask], [int(max(binNum)+1)]*len(x1[~mask])])
				binned_array3 = np.row_stack([binned_array2, binned_array1])
				np.savetxt(output_filename, binned_array3, fmt=b'%10.6f %10.6f %8i')
				bf.print_cust('Saved...', quiet_val=quiet_val)
		else:
...
def binning_result_display(input_filename, output_filename, par_dict, snr_type='snr', physical_axes=False, snr_vmin_val=0, snr_vmax_val=20, size_of_font=10, quiet_val=False):
	bf.print_cust(f"Loading file: {output_filename} for plotting...", quiet_val=quiet_val)
	...
	if ('none' not in str(par_dict['binning_type'].lower())):
		...
		bf.print_cust('Plotting Figure 1...', quiet_val=quiet_val)
		...
		plt.show()
		bf.print_cust('Plotted Figure 1...', quiet_val=quiet_val)
	bf.print_cust(f"Loading file: {input_filename} for plotting...", quiet_val=quiet_val)
	y, x, ra, dec, signal1, noise1, signal2, noise2 = np.loadtxt(input_filename).T
	if ('ew' in snr_type.lower()):
		signal = signal2
		noise = noise2
	...
	snr = signal/noise
	bf.print_cust(f"SNR/EW Limits: {np.min(snr)} to {np.max(snr)}", quiet_val=quiet_val)
	bf.print_cust('Plotting Figure 2...', quiet_val=quiet_val)
	...
	axs[0].set_title("SNR")
	bf.add_colorbar_lin(im1)
	im2 = axs[1].scatter(x_new, y_new, c = signal_new, cmap='RdBu')
	axs[1].set_title("Binning")
	bf.add_colorbar_lin(im2)
	plt.show()
	bf.print_cust('Plotted Figure 2...', quiet_val=quiet_val)
######################DISPLAY_BINNING_RESULT##################################
...
def create_linear_file_for_binning(file_name_rev_linear, par_dict, header_rev, cont_rev_file2, data_rev_file2, err_rev_file2, data_ew_original_file2, err_ew_original_file2, quiet_val=False):
	bf.print_cust('Creating linear data file for binning...', quiet_val=quiet_val)
	ra_rev, dec_rev, wave_rev = hff.obtain_physical_axis(header_rev)
	center_position_idx = bf.find_nearest_idx(cont_rev_file2[:, int(cont_rev_file2.shape[1]/2), int(cont_rev_file2.shape[2]/2)],  np.nanmax(cont_rev_file2[:, int(cont_rev_file2.shape[1]/2), int(cont_rev_file2.shape[2]/2)]))
	...
	data_ew_original_file = bf.clean_data(data_ew_original_file2)
	err_ew_original_file = bf.clean_data(err_ew_original_file2, type_of_data='err')
	data_ew_original_file[np.abs(data_ew_original_file)>par_dict['max_ew_width_abs']] = 1e-6
	err_ew_original_file[np.abs(data_ew_original_file)>par_dict['max_ew_width_abs']] = 1e-5
	...
	for i in range(data_rev_file2.shape[1]):
		for j in range(data_rev_file2.shape[2]):
			bar.next()
			idx_new = bf.arg_median(data_rev_file2[idx1:idx2,i,j])
			lin_x_axis[i,j] = i+1
			lin_y_axis[i,j] = j+1
	...
	bf.print_cust('Linear data file for binning created...', quiet_val=quiet_val)
####################CREATING_LINEAR_DATA_FILE_FOR_BINNING####################
